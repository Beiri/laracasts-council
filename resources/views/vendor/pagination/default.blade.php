    ...
    <div class="text-center">
        <ul class="inline-flex w-48 list-reset border py-2 mt-4 mb-8 rounded">
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <li class="flex-1 font-bold border-r"><a href="{{ $url }}"
                                    class="text-blue">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @if ($paginator->hasMorePages())
                <li class="flex-1"><a href="{{ $paginator->nextPageUrl() }}" rel="next"
                        class="text-blue font-bold">&raquo;</a></li>
            @else
