        ...
        <div class="flex-1 {{ $loop->last ? '' : 'border-b border-blue-lightest' }}">
            ...
            <p class="text-2xs text-grey-darkest mb-4">
                Posted By: <a href="{{ route('profile', $thread->creator) }}"
                    class="text-blue">{{ $thread->creator->username }}</a>
            </p>

            <thread-view :thread="{{ $thread }}" inline-template
                class="mb-6 text-grey-darkest leading-loose pr-8">
            ...
            <div class="flex items-center text-xs mb-6">
                ...
                <a class="btn ml-auto is-outlined text-grey-darker py-2 text-xs" href="{{ $thread->path() }}">read
                    more</a>
            </div>
        </div>
    </div>
