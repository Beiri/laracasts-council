...
<div class="">
    <div class="flex">
        <img src="{{ $thread->creator->avatar_path }}" alt="{{ $thread->creator->username }}" width="36"
            height="36" class="mr-1 bg-blue-darker rounded-full p-2">
                ...
                <span v-if="! editing">
                    <span v-if="(authorize('isAdmin') || authorize('owns', thread))">
                        <a href="#" class="text-blue link pl-2 ml-2 border-l"
                            @click.prevent="editing = true">Edit</a>
                        <a href="#" class="link pl-2 ml-2 border-l" :class="locked ? 'font-bold' : ''"
                            @click.prevent="toggleLock" v-text="locked ? 'Unlock' : 'Lock'"></a>
                        <a href="#" class="link pl-2 ml-2 border-l" :class="pinned ? 'font-bold' : ''"
                            @click.prevent="togglePin" v-text="pinned ? 'Unpin' : 'Pin'"></a>
                    </span>
