<template>
  <div>
    ...
    <div
      class="timeline relative w-full max-w-full border-l-4 border-grey-light"
    >
      <div v-for="(activity, index) in items" :key="activity.id">
        <div class="entry">
          <activity-favorite
            :activity="activity"
            v-if="activity.type === 'created_favorite'"
          />
          <activity-reply
            :activity="activity"
            v-if="activity.type === 'created_reply'"
          />
          <activity-thread
            :activity="activity"
            v-if="activity.type === 'created_thread'"
          />
        </div>
      </div>
    </div>
